...
def cxttsvm():
    messagebox.showinfo('Độ chính xác của thuật toán svm', cxptsvm)

def cxttrd():
    messagebox.showinfo('Độ chính xác của thuật toán rd', cxptrd)


cxsvm = Button(windown, text='Độ chính xác thuật toán svm', command=cxttsvm).pack(side = TOP, fill = BOTH)
cxrd = Button(windown, text='Độ chính xác thuật toán rd', command=cxttrd).pack(side = TOP, fill = BOTH)
